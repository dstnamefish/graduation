 ...
 * @description 表格配置接口
 * @property core - 核心配置
 * @property transform - 数据转换配置
 * @property performance - 性能优化配置
 * @property hooks - 钩子函数配置
 * @property debug - 调试配置
 */
export interface UseTableConfig<TApiFn extends (params: any) => Promise<any>, TRecord = InferRecordType<InferApiResponse<TApiFn>>, TParams = InferApiParams<TApiFn>, TResponse = InferApiResponse<TApiFn>> {

  /**
   * @description 核心配置
   * @property apiFn - API 请求函数，用于获取数据
   * @property apiParams - API 默认请求参数，用于分页等
   * @property columnsFactory - 列配置工厂函数，用于动态生成列配置
   * @property excludeParams - 排除的apiParams参数，用于避免在 API 调用中包含某些参数
   * @property immediate - 是否立即调用 API 函数
   * @property paginationKey - 分页参数键名，用于指定当前页和每页数量的参数名
   */
  core: {
    apiFn: TApiFn;
  ...
  /**
   * @description 数据转换配置
   * @property dataTransformer - 数据转换函数，用于对从 API 获取的数据进行转换
   * @property responseAdapter - 响应适配器函数，用于对 API 响应进行适配
   */
  transform?: {
  ...
  /**
   * @description 性能优化
   * @property cacheTime - 缓存时间，单位毫秒，默认 5 分钟
   * @property debounceTime - 防抖时间，单位毫秒，默认 300 毫秒
   * @property enableCache - 是否启用缓存，默认 false
   * @property maxCacheSize - 最大缓存大小，默认 50 条记录
   */
  performance?: {
    cacheTime?: number;
    debounceTime?: number;
  ...
  /**
   * @description 钩子函数配置
   * @property onCacheHit - 缓存命中回调函数，用于在缓存命中时触发
   * @property onError - 错误回调函数，用于在 API 调用出错时触发
   * @property onLoading - 加载状态回调函数，用于在 API 调用开始和结束时触发
   * @property onSuccess - 成功回调函数，用于在 API 调用成功时触发
   * @property resetFormCallback - 重置表单回调函数，用于在重置表单时触发
   */
  hooks?: {
  ...
  /**
   * @description 调试配置
   * @property enableLog - 是否启用日志记录，默认 false
   * @property logLevel - 日志级别，默认 info
   */
  debug?: {
    enableLog?: boolean;
...
/**
 * @description 表格 Hook 函数，用于创建和管理表格数据 采用外观模式，提供简单的 API 接口
 * @param config 表格配置
 * @returns 表格实例
 */
...
function useTableImpl<TApiFn extends (params: any) => Promise<any>>(config: UseTableConfig<TApiFn>) {
  ...
  const {
    core: { apiFn, apiParams = {} as Partial<TParams>, columnsFactory, excludeParams = [], immediate = true, paginationKey = { current: 'current', size: 'size' } },
    debug: { enableLog = false } = {},
    hooks: { onCacheHit, onError, onSuccess, resetFormCallback } = {},
    performance: { cacheTime = 5 * 60 * 1000, debounceTime = 300, enableCache = false, maxCacheSize = 50 } = {},
  ...
  const cacheInfo = computed(() => {
    ...
    if (!cache) {
      return { hitRate: '0 avg hits', size: '0KB', total: 0 };
    }
  ...
  const fetchData = async (params?: Partial<TParams>, useCache = enableCache): Promise<ApiResponse<TRecord>> => {
    ...
    try {
      ...
      if (err instanceof Error && err.message === '请求已取消') {
        // 请求被取消，不做处理
        return { current: 1, records: [], size: 10, total: 0 };
      }
