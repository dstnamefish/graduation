    ...
    >
      ...
      <div class="input-container">
        ...
        <input
          ...
          :autocomplete="field.name === 'password' ? 'current-password' : (field.name === 'username' ? 'username' : undefined)"
      ...
      <div
        v-if="
          fieldErrors[field.name] && (touchedFields[field.name] || isSubmitted)
        "
        class="error-message"
      >
...
defineOptions({ name: 'WellnestFormAuth' });
...
const validateField = async (fieldName: string) => {
  ...
  if (!fieldError && field.validator && value) {
    try {
      fieldError = await executeValidator(
        field.validator,
        value,
        field,
        formData,
